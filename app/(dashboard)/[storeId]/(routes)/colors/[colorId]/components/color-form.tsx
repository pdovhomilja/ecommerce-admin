...
const formSchema = z.object({
  ...
  value: z
    ...
    .regex(/^#([0-9a-f]{3}){1,2}$/i, {
      message: "String must be a valid color value",
    }),
});
...
const ColorForm = ({ initialData }: ColorFormProps) => {
  ...
  //Action triggered when the form is submitted to update the store settings
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error("Make sure you have no products using this color first");
    } finally {
      setLoading(false);
      setOpen(false);
    }
